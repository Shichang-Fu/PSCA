...
def make_optimizer(cfg, model, name):
    ...
    for key, value in model.named_parameters():
        ...
        if name == 'backbone':
            ...
            lr = cfg.SOLVER.MEMORY.BASE_LR
            weight_decay = cfg.SOLVER.WEIGHT_DECAY
            if "bias" in key:
                lr = cfg.SOLVER.MEMORY.BASE_LR * cfg.SOLVER.MEMORY.BIAS_LR_FACTOR
                weight_decay = cfg.SOLVER.WEIGHT_DECAY_BIAS
        elif name == 'fusion':
            lr = cfg.SOLVER.FUSION.BASE_LR
            weight_decay = cfg.SOLVER.WEIGHT_DECAY
            if "bias" in key:
                lr = cfg.SOLVER.FUSION.BASE_LR * cfg.SOLVER.FUSION.BIAS_LR_FACTOR
                weight_decay = cfg.SOLVER.WEIGHT_DECAY_BIAS
        else:
...
def make_lr_scheduler(cfg, optimizer, name):
    if name == 'backbone':
        ...
        return WarmupMultiStepLR(
            optimizer,
            cfg.SOLVER.MEMORY.STEPS,
            cfg.SOLVER.MEMORY.GAMMA,
            warmup_factor=cfg.SOLVER.MEMORY.WARMUP_FACTOR,
            warmup_iters=cfg.SOLVER.MEMORY.WARMUP_ITERS,
            warmup_method=cfg.SOLVER.MEMORY.WARMUP_METHOD,
        )
    elif name == 'fusion':
        return WarmupMultiStepLR(
            optimizer,
            cfg.SOLVER.FUSION.STEPS,
            cfg.SOLVER.FUSION.GAMMA,
            warmup_factor=cfg.SOLVER.FUSION.WARMUP_FACTOR,
            warmup_iters=cfg.SOLVER.FUSION.WARMUP_ITERS,
            warmup_method=cfg.SOLVER.FUSION.WARMUP_METHOD,
        )
    else:
        raise AssertionError('here')